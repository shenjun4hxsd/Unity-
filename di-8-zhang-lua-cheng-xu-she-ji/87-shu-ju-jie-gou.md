...
&emsp;&emsp;每个结点表示为一个table，这个table有两个字段：name（结点的名称）和adj（与此结点邻接的结点集合）。由于从一个文本文件中读取图数据，所以需要一种通过一个结点名来找到该结点的方法。因此，使用了一个额外的table来将名称对应的结点。函数name2node可以根据给定的名称返回对应的结点：
