...
&emsp;&emsp;虽然对稀疏矩阵使用长度操作符不是一种语法错误。但也不能进行该操作，因为在有效条目之间存在“空洞（nil值）”。在大多数对稀疏矩阵的操作中，由于存在许多空条目，遍历矩阵是非常低效的。所以，一般使用pairs且只遍历那些非nil的元素。例如，要将矩阵的一行与一个常量相乘，可以使用以下代码：
