...
&emsp;&emsp;Lua5对这个问题进行了改进，它允许每个函数拥有一个自己的环境来查找全局变量。第一次听到这项机制可能会感觉不理解，毕竟一个用于记录全局变量的table本身也应该是全局的。在后续章节会看到几种基于该机制的技术，它们能使全局变量访问任何地方。
...
&emsp;&emsp;每个函数及某些closure都有一个继承的环境。下面这段代码就演示了这种机制：
