...
&emsp;&emsp;因此，假设迭代器函数为f，恒定状态为s，控制变量的初值为a<sub>0</sub>。那么在循环过程中控制变量的值依次为$$a_1$$ = f(s, a<sub>0</sub>、a<sub>2</sub> = f(s, a<sub>1</sub>))，以此类推，直至a<sub>i</sub>为nil结束循环。如果for还有其他变量，那么它们也会在每次调用f后获得额外的值。